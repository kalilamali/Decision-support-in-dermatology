...
def get_weights(train):
    ...
    # Reproducibility
    myseed(seed=42)
    #counts = list(train['label'].value_counts())
    counts_df = train.label.value_counts().to_frame().reset_index().sort_values(by='index').set_index('index')
    counts = list(counts_df['label'])
    # Min freq balancing
    min_freq = np.min(counts)
    weight = [safe_division(min_freq, x) for x in counts]
    #summed = sum(counts)
    #weight = [safe_division(x, summed) for x in counts]
    #weight = [safe_division(1, x) for x in weight]
    #weight = [safe_division(x, summed) for x in weight]
